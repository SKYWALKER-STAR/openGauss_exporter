...
const (
	...
	// See: https://github.com/go-sql-driver/gauss#system-variables
...
var (
	...
	slowLogFilter = kingpin.Flag(
		...
		"Add a log_slow_filter to avoid slow query logging of scrapes. NOTE: Not supported by Oracle GaussDB.",
	).Default("false").Bool()
)

// metric definition
var (
	gaussUp = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, "", "up"),
		"Whether the GaussDB server is up.",
		nil,
		nil,
	)
	gaussScrapeCollectorSuccess = prometheus.NewDesc(
		prometheus.BuildFQName(namespace, exporter, "collector_success"),
		"gaussd_exporter: Whether a collector succeeded.",
		[]string{"collector"},
		nil,
	)
	gaussScrapeDurationSeconds = prometheus.NewDesc(
...
// Exporter collects Gauss metrics. It implements prometheus.Collector.
...
func (e *Exporter) Describe(ch chan<- *prometheus.Desc) {
	ch <- gaussUp
	ch <- gaussScrapeDurationSeconds
	ch <- gaussScrapeCollectorSuccess
}

// Collect implements prometheus.Collector.
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	up := e.scrape(e.ctx, ch)
	ch <- prometheus.MustNewConstMetric(gaussUp, prometheus.GaugeValue, up)
}
...
func (e *Exporter) scrape(ctx context.Context, ch chan<- prometheus.Metric) float64 {
	...
	if err := db.PingContext(ctx); err != nil {
		level.Error(e.logger).Log("msg", "Error pinging gaussdb", "err", err)
		return 0.0
	}
	...
	for _, scraper := range e.scrapers {
		...
		*/
		...
		go func(scraper Scraper) {
			...
			ch <- prometheus.MustNewConstMetric(gaussScrapeCollectorSuccess, prometheus.GaugeValue, collectorSuccess, label)
			ch <- prometheus.MustNewConstMetric(gaussScrapeDurationSeconds, prometheus.GaugeValue, time.Since(scrapeTime).Seconds(), label)
		}(scraper)
	}
	return 1.0
}
...
func (e *Exporter) getTargetFromDsn() string {
	// Get target from DSN.
	dsnConfig, err := gauss.ParseDSN(e.dsn)
	if err != nil {
...
func getGaussDBVersion(db *sql.DB, logger log.Logger) float64 {
