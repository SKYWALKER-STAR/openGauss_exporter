...
func (PGStatWalReceiverCollector) Name() string {
	return "stat wal receiver"
}

func (PGStatWalReceiverCollector) Help() string {
	return "stat wal information"
}

func (PGStatWalReceiverCollector) Version() float64 {
	return 1.0
}

func (PGStatWalReceiverCollector) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric,logger log.Logger) error {
	...
	for rows.Next() {
		...
		if !upstreamHost.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because upstream host is null")
			continue
		}

		if !slotName.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because slotname host is null")
			continue
		}

		if !status.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because status is null")
			continue
		}
		labels := []string{upstreamHost.String, slotName.String, status.String}

		if !receiveStartLsn.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because receive_start_lsn is null")
			continue
		}
		if !receiveStartTli.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because receive_start_tli is null")
			continue
		}
		if hasFlushedLSN && !flushedLsn.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because flushed_lsn is null")
			continue
		}
		if !receivedTli.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because received_tli is null")
			continue
		}
		if !lastMsgSendTime.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because last_msg_send_time is null")
			continue
		}
		if !lastMsgReceiptTime.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because last_msg_receipt_time is null")
			continue
		}
		if !latestEndLsn.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because latest_end_lsn is null")
			continue
		}
		if !latestEndTime.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because latest_end_time is null")
			continue
		}
		if !upstreamNode.Valid {
			level.Debug(logger).Log("msg", "Skipping wal receiver stats because upstream_node is null")
			continue
		}
